...
export default function JungleRunHomePage() {
  const router = useRouter();
  const [selectedFee, setSelectedFee] = useState<number | null>(50);

  const handlePlay = () => {
    if (selectedFee !== null) {
      router.push(`/games/jungle_run/play?fee=${selectedFee}`);
    }
  };
  
  const gameBanner = PlaceHolderImages.find(p => p.id === 'jungle_run');
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
        <div>
            <Card className="overflow-hidden mb-4 border-primary/50">
                ...
                <Image
                    src={gameBanner?.imageUrl || "https://picsum.photos/seed/jungle_run/600/400"}
                    alt={"Jungle Run Game"}
                    fill
