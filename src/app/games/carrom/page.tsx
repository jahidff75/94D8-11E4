...
const feeOptions = [
  { fee: 20, prize: 70, players: 2 },
  { fee: 50, prize: 180, players: 2 },
  { fee: 100, prize: 350, players: 2 },
  { fee: 150, prize: 530, players: 2 },
  { fee: 200, prize: 700, players: 2 },
  { fee: 300, prize: 1050, players: 2 },
  { fee: 500, prize: 1750, players: 2 },
];

export default function CarromHomePage() {
  const router = useRouter();
  const [selectedFee, setSelectedFee] = useState<number | null>(100);

  const handlePlay = () => {
    if (selectedFee !== null) {
      router.push(`/games/carrom/play?fee=${selectedFee}`);
    }
  };
  
  const gameBanner = PlaceHolderImages.find(p => p.id === 'carrom');
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 text-white">
      <header className="flex items-center justify-between p-2 border-b border-gray-700 sticky top-0 bg-background/90 backdrop-blur-sm z-10">
        ...
        <h1 className="text-xl font-bold">Carrom</h1>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
        <div>
            ...
            <Card className="overflow-hidden mb-4 border-primary/50">
                ...
                <Image
                    src={gameBanner?.imageUrl || "https://picsum.photos/seed/carrom/600/400"}
                    alt={"Carrom Game"}
                    fill
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-4 flex flex-col justify-end">
                    <h2 className="text-3xl font-bold text-white drop-shadow-lg">DOXPOW CARROM</h2>
