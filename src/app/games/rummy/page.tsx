...
export default function RummyHomePage() {
  ...
  const handlePlay = () => {
    if (selectedFee !== null) {
      router.push(`/games/rummy/play?fee=${selectedFee}`);
    }
  };
  
  const gameBanner = PlaceHolderImages.find(p => p.id === 'rummy');
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 text-white">
      <header className="flex items-center justify-between p-2 border-b border-gray-700 sticky top-0 bg-background/90 backdrop-blur-sm z-10">
        ...
        <h1 className="text-xl font-bold">Rummy</h1>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
        <div>
            <Card className="overflow-hidden mb-4 border-primary/50">
                ...
                <Image
                    src={gameBanner?.imageUrl || "https://picsum.photos/seed/rummy/600/400"}
                    alt={"Rummy Game"}
                    fill
