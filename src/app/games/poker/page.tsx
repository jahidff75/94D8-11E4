...
const feeOptions = [
  ...
  { fee: 200, prize: 700, players: 5 },
  { fee: 500, prize: 1750, players: 5 },
  { fee: 1000, prize: 3500, players: 5 },
];
...
export default function PokerHomePage() {
  ...
  const handlePlay = () => {
    if (selectedFee !== null) {
      router.push(`/games/poker/play?fee=${selectedFee}`);
    }
  };
  
  const gameBanner = PlaceHolderImages.find(p => p.id === 'poker');
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
        <div>
            <Card className="overflow-hidden mb-4 border-primary/50">
                ...
                <Image
                    src={gameBanner?.imageUrl || "https://picsum.photos/seed/poker/600/400"}
                    alt={"Poker Game"}
                    fill
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-4 flex flex-col justify-end">
                    <h2 className="text-3xl font-bold text-white drop-shadow-lg">DOXPOW POKER</h2>
