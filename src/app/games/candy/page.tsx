...
export default function CandyHomePage() {
  const router = useRouter();
  const [selectedFee, setSelectedFee] = useState<number | null>(50);

  const handlePlay = () => {
    if (selectedFee !== null) {
      router.push(`/games/candy/play?fee=${selectedFee}`);
    }
  };
  
  const gameBanner = PlaceHolderImages.find(p => p.id === 'candy');
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 text-white">
      <header className="flex items-center justify-between p-2 border-b border-gray-700 sticky top-0 bg-background/90 backdrop-blur-sm z-10">
        ...
        <h1 className="text-xl font-bold">Candy Crush</h1>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
        <div>
            <Card className="overflow-hidden mb-4 border-primary/50">
                ...
                <Image
                    src={gameBanner?.imageUrl || "https://picsum.photos/seed/candy/600/400"}
                    alt={"Candy Crush Game"}
                    fill
                    className="object-cover"
                    data-ai-hint={gameBanner?.imageHint}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-4 flex flex-col justify-end">
                    <h2 className="text-3xl font-bold text-white drop-shadow-lg">DOXPOW CANDY CRUSH</h2>
