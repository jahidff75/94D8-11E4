...
export default function MetroHomePage() {
  const router = useRouter();
  const [selectedFee, setSelectedFee] = useState<number | null>(100);

  const handlePlay = () => {
    if (selectedFee !== null) {
      router.push(`/games/metro/play?fee=${selectedFee}`);
    }
  };
  
  const gameBanner = PlaceHolderImages.find(p => p.id === 'metro');
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
        <div>
            <Card className="overflow-hidden mb-4 border-primary/50">
                ...
                <Image
                    src={gameBanner?.imageUrl || "https://picsum.photos/seed/metro/600/400"}
                    alt={"Metro Surface Game"}
                    fill
                    className="object-cover"
                    data-ai-hint={gameBanner?.imageHint}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-4 flex flex-col justify-end">
                    <h2 className="text-3xl font-bold text-white drop-shadow-lg">DOXPOW METRO</h2>
